...
| P0-B — auth/certificate/abuse/Vercel | Hoàn tất code và local smoke; deployment bị chặn vận hành | Content POST bị vô hiệu hóa, generate-certificate cũ trả 410, verify certificate đọc backend canonical, Turnstile/sync fail closed, grant role giới hạn, unknown API trả JSON 404, `vercel build --prod` thành công | Ba cách deploy đều tạo deployment `UNKNOWN`; alias chính vẫn ở bản cũ và health còn trả HTML. Cần xử lý Vercel trước khi smoke production. Không coi Firebase rules đã deploy theo Vercel. |
