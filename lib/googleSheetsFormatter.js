...
const COLORS = {
  ...
  wineRed: { red: 0.42, green: 0.12, blue: 0.18 },
...
async function writeTable(spreadsheetId, sheetTitle, headers, rows, statusColIndex, bdhRowIndices = []) {
  const colCount = headers.length;
  const sheetId = await ensureSheet(spreadsheetId, sheetTitle, colCount);

  const safeTitle = `'${sheetTitle.replace(/'/g, "''")}'`;
  await clearRange(spreadsheetId, `${safeTitle}!A1:ZZ99999`);

  const values = [headers, ...rows];
  await appendRows(spreadsheetId, `${safeTitle}!A1`, values);
  ...
  if (dataRowCount > 1) {
    ...
    // Apply full RED fill background to BĐH / Vice rows
    if (Array.isArray(bdhRowIndices) && bdhRowIndices.length > 0) {
      bdhRowIndices.forEach(rIdx => {
        requests.push({
          repeatCell: {
            range: { sheetId, startRowIndex: rIdx, endRowIndex: rIdx + 1, startColumnIndex: 0, endColumnIndex: colCount },
            cell: {
              userEnteredFormat: {
                backgroundColor: COLORS.wineRed || { red: 0.42, green: 0.12, blue: 0.18 },
