...
async function addGoogleCalendarAttendees(event, candidateEmail, hrEmail, adminEmails) {
...
async function sendFinalizedInvitation(event, candidate, hr, adminEmails) {
    ...
    const adminRecipients = Array.from(new Set((Array.isArray(adminEmails) ? adminEmails : [])
        .map(email => String(email || '').trim().toLowerCase())
        .filter(email => email && email !== String(candidate.email || '').trim().toLowerCase() && email !== String(hr.email || '').trim().toLowerCase())));
    await Promise.all([
        ...
        ...adminRecipients.map(email => transporter.sendMail({
            from, to: email,
            subject: `[Admin] Lịch phỏng vấn — ${candidate.name || candidate.email}`,
            html: wrap(`<p>Chào Admin,</p><p>Bạn được thêm vào lịch để có thể tham dự hoặc hỗ trợ buổi phỏng vấn.</p>${details}`)
        }))
    ]);
}
        ...
        for (const item of plan) {
            ...
            try {
                ...
                if (!event.location) {
                    ...
                    // Lịch đã tồn tại từ lần chạy trước cũng được bổ sung toàn bộ Admin.
                ...
                if (!event.invitationSentAt) {
                    await sendFinalizedInvitation(event, candidate, hr, adminEmails);
