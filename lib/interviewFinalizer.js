...
function normalizedVoteSlots(schedule) {
    const raw = Array.isArray(schedule && schedule.slots) ? schedule.slots.map(String) : [];
    if (schedule && schedule.slotSchema === '30m-v1') return raw;
    const result = new Set();
    raw.forEach(slotId => {
        const parts = slotId.split('_');
        const index = Number(parts[1]);
        if (parts[0] && Number.isInteger(index) && index >= 0 && index < 6) {
            for (let offset = 0; offset < 3; offset += 1) result.add(`${parts[0]}_${index * 3 + offset}`);
        } else result.add(slotId);
    });
...
async function createGoogleMeet(eventId, event, candidateEmail, hrEmail, adminEmails) {
    const token = await getGoogleAccessToken();
    const calendarId = process.env.GOOGLE_CALENDAR_ID || 'primary';
    const start = new Date(event.startAt);
    const end = new Date(start.getTime() + Number(event.duration || 30) * 60000);
    // Admin vẫn được thêm vào danh sách attendee để vào Meet khi cần; sendUpdates=none
    // ở dưới bảo đảm Google Calendar không gửi thư mời cho họ.
    const attendees = Array.from(new Set([candidateEmail, hrEmail].concat(Array.isArray(adminEmails) ? adminEmails : []).filter(Boolean).map(email => email.toLowerCase())))
        .map(email => ({ email }));
    const resource = {
        summary: event.title,
    ...
    // Thư xác nhận được gửi chủ động qua Brevo cho đúng ứng viên và người phỏng vấn.
    // Không để Google Calendar tự gửi thư cho các attendee khác.
...
async function addGoogleCalendarAttendees(event, candidateEmail, hrEmail, adminEmails, removeEmails) {
    ...
    (Array.isArray(removeEmails) ? removeEmails : []).forEach(email => {
        const value = String(email || '').trim().toLowerCase();
        if (value) emails.delete(value);
    });
    [candidateEmail, hrEmail].concat(Array.isArray(adminEmails) ? adminEmails : []).forEach(email => {
        const value = String(email || '').trim().toLowerCase();
        if (value) emails.add(value);
    });
    const response = await fetch(`${baseUrl}?sendUpdates=none`, {
        method:'PATCH',
        headers:{ Authorization:`Bearer ${token}`, 'Content-Type':'application/json' },
...
function buildPlan({ poll, schedules, users, applications, existingEvents, bookings, now = new Date() }) {
    ...
    const feasibility = candidateSchedules.filter(candidate => !scheduledCandidates.has(candidate.ownerId)).map(candidate => {
        ...
        const candidateSlots = new Set(normalizedVoteSlots(candidate));
    ...
    for (const poll of polls.filter(item => item.type === 'interview' && item.status === 'open')) {
        ...
        for (const item of plan) {
            ...
            try {
                ...
                if (!event.location) {
                    const calendar = await createGoogleMeet(eventId, event, candidate.email, hr.email, adminEmails);
                    ...
                    await addGoogleCalendarAttendees(event, candidate.email, hr.email, adminEmails);
                }
