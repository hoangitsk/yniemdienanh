...
function buildPlan({ poll, schedules, users, applications, existingEvents, bookings, now = new Date() }) {
    ...
    const candidateSchedules = schedules.filter(item => item.role === 'candidate' && item.ownerId);
    const hrSchedules = schedules.map(schedule => ({ schedule, account: usersById.get(schedule.ownerId) }))
        .filter(entry => entry.schedule.role === 'btc' && entry.account && (isHrUser(entry.account) || entry.account.id === adminUser?.id));
    const bookedEventIds = new Set(bookings.filter(item => item.status === 'confirmed').map(item => item.eventId));
    const pollEvents = existingEvents.filter(event => event.availabilityPollId === poll.id && event.status !== 'cancelled');
    const completedEvents = pollEvents.filter(event => bookedEventIds.has(event.id) && event.status === 'confirmed' && event.invitationSentAt);
    const scheduledCandidates = new Set(completedEvents.map(event => event.candidateId));
...
module.exports = { SHIFTS, addDays, buildPlan, finalizeInterviews, slotInfo, vnDateId };
