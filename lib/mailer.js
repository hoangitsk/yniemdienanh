...
function availableSettings(preferredProvider) {
    const settings = [brevoSettings(), gmailSettings()].filter(Boolean);
    if (!preferredProvider) return settings;
    return settings.sort(function(a, b) {
        return (a.provider === preferredProvider ? 0 : 1) - (b.provider === preferredProvider ? 0 : 1);
    });
}

function preferredSender(preferredProvider) {
    const settings = availableSettings(preferredProvider)[0];
    return settings ? {
...
async function sendMailWithFallback(message, options) {
    const settings = availableSettings(options && options.preferredProvider);
    if (!settings.length) {
