    ...
    try {
        const result = await db.runTransaction(async transaction => {
            ...
            if (tx.type === 'registration') {
                ...
                label = `Thu phí thi đội: ${tx.eventTitle || tx.eventId} - ${tx.userName || ''}`;
            } else if (tx.donationType === 'prize') {
                label = `Donate Quỹ giải thưởng - ${tx.userName || ''}`;
            } else if (tx.donationType === 'operation') {
                label = `Donate Vận hành dự án - ${tx.userName || ''}`;
            } else if (tx.sponsorTier) {
                label = `Tài trợ ${String(tx.sponsorTier).toUpperCase()} - ${tx.userName || ''}`;
            } else {
                label = `Đóng góp chung - ${tx.userName || ''}`;
            }
