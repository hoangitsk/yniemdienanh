...
function rankingColumns(headers) {
  const cols = {};
  headers.forEach((h, i) => {
    const norm = normalizeText(h).replace(/_/g, ' ');
    if (norm.includes('ten') || norm.includes('name')) cols.name = i;
    else if (norm.includes('chuc vu') || norm.includes('chuc danh') || norm.includes('vai tro') || norm.includes('role') || norm.includes('vi tri') || norm.includes('position')) cols.role = i;
    else if (norm.includes('email') || norm.includes('mail') || norm.includes('thu dien tu')) cols.email = i;
    else if (norm.includes('so dien thoai') || norm.includes('phone') || norm.includes('sdt') || norm.includes('dien thoai')) cols.phone = i;
    else if (norm.includes('facebook') || norm.includes('fb')) cols.facebook = i;
    else if (norm.includes('ghi chu') || norm.includes('note')) cols.note = i;
    else if (norm === 'ban' || norm.includes('phong ban') || norm.includes('bo phan') || norm.includes('department')) cols.ban = i;
    else if (norm.includes('gioi tinh') || norm.includes('gender')) cols.gender = i;
    else if (norm.includes('ngay sinh') || norm.includes('dob')) cols.dob = i;
    else if (norm.includes('truong') || norm.includes('school')) cols.school = i;
    else if (norm.includes('noi sinh song') || norm.includes('dia chi') || norm.includes('address')) cols.address = i;
  });
  return cols;
}

// Tìm dòng header thật trong tab ranking — dòng 1 có thể là banner
// ("DATABASE CORE TEAM – Ý NIỆM ĐIỆN ẢNH") chứ không phải header.
function findRankingHeaderRow(rows) {
  const limit = Math.min(rows.length, 10);
  for (let r = 0; r < limit; r++) {
    const cols = rankingColumns(rows[r] || []);
    if (cols.name != null && cols.email != null) return { row: r, cols };
  }
  return null;
}
...
async function readRankingMembers(sheets) {
  ...
  try {
    ...
    // Ưu tiên đọc các tab Core/BTC trước, sau đó đến Thành viên
    ...
    for (const tab of tabsToRead) {
      ...
      for (let i = headerRow + 1; i < rows.length; i++) {
        ...
        if (!name) continue;
        const email = cols.email != null ? String(row[cols.email] || '').trim().toLowerCase() : '';
        if (!email || !email.includes('@')) continue;
        if (cols.ban != null && String(row[cols.ban] || '').trim()) {
          currentBan = String(row[cols.ban] || '').trim();
        }
        const rawRole = cols.role != null ? String(row[cols.role] || '').trim() : '';
        ...
        const memberObj = {
          ...
          facebook: cols.facebook != null ? String(row[cols.facebook] || '') : '',
          notes: cols.note != null ? String(row[cols.note] || '') : '',
          school: cols.school != null ? String(row[cols.school] || '') : '',
          address: cols.address != null ? String(row[cols.address] || '') : '',
          gender: cols.gender != null ? String(row[cols.gender] || '') : '',
          dob: cols.dob != null ? String(row[cols.dob] || '') : '',
          tab
        };
        ...
        if (!seenEmails.has(email)) {
          ...
          // Nếu đã thấy nhưng tab mới là Core/Executive và bản ghi trước là Member -> Nâng cấp
          const existingIdx = out.findIndex(m => m.email === email);
...
function departmentFromName(name) {
  ...
  if (/core (hr|nhan su)/i.test(n)) return 'NHAN_SU';
  if (/core (comms|truyen thong)/i.test(n)) return 'TRUYEN_THONG';
  if (/core (media)/i.test(n)) return 'MEDIA';
  if (/core (noi dung)/i.test(n)) return 'NOI_DUNG';
  if (/core (duyet)/i.test(n)) return 'DUYET_BAI';
  return 'GLOBAL';
}

function departmentFromRoleTitle(v) {
  const t = normalizeText(v);
  if (/hr|nhan su/.test(t)) return 'NHAN_SU';
  if (/comms|truyen thong/.test(t)) return 'TRUYEN_THONG';
  if (/media/.test(t)) return 'MEDIA';
  if (/noi dung/.test(t)) return 'NOI_DUNG';
  if (/duyet/.test(t)) return 'DUYET_BAI';
  return '';
}
...
function departmentFromBan(b) {
  ...
  if (t.includes('nhan su') || t.includes('hr')) return 'NHAN_SU';
  if (t.includes('truyen thong') || t.includes('comms')) return 'TRUYEN_THONG';
  if (t.includes('media')) return 'MEDIA';
  if (t.includes('noi dung')) return 'NOI_DUNG';
  if (t.includes('duyet')) return 'DUYET_BAI';
  if (t.includes('ban dieu hanh') || t.includes('bdh') || t.includes('dieu hanh') || t.includes('giam doc')) return 'GLOBAL';
  return '';
}
