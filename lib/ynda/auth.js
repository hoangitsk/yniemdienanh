...
function mapRankingRole(rawRole, tabName = '') {
  const v = String(rawRole || '').trim();
  const t = v.toLowerCase().normalize('NFD').replace(/[\u0300-\u036f]/g, '').replace(/[đĐ]/g, 'd');
  const tabNorm = String(tabName || '').toLowerCase().normalize('NFD').replace(/[\u0300-\u036f]/g, '').replace(/[đĐ]/g, 'd');

  if (/founder|sang lap|sang lap vien/.test(t) && !/dong sang lap|co founder/.test(t)) return 'FOUNDER';
  if (/president|chu tich/.test(t)) return 'PRESIDENT';
  if (/co founder|dong sang lap/.test(t)) return 'CO_FOUNDER';
  if (/co van|advisor|tham van|chuyen mon/.test(t)) return 'ADVISOR';
  if (/pho ban|vice|deputy|vice lead|pho lead|pho nhom|pho/.test(t)) return 'VICE';
...
function canonicalDept(dept, rawRole = '', role = '', name = '') {
  ...
  if (d) {
    if (/co van|advisor|tham van|chuyen mon/.test(d)) return 'Cố Vấn Chuyên Môn';
    if (/media|hau ky|video|design|edit|san xuat/.test(d)) return 'Ban Media';
  ...
  if (r && !/^(member|thanh vien|ctv)$/.test(r)) {
    if (/co van|advisor|tham van|chuyen mon/.test(r)) return 'Cố Vấn Chuyên Môn';
    if (/media|hau ky|video|design|edit|san xuat/.test(r)) return 'Ban Media';
  ...
  if (n) {
    ...
    if (/to uyen/.test(n)) return 'Cố Vấn Chuyên Môn';
    if (/thanh thao|thanh truc/.test(n)) return 'Ban Media';
    ...
    try {
      ...
      for (const tab of tabsToRead) {
        ...
        for (let i = headerRow + 1; i < rows.length; i++) {
          ...
          // Cập nhật currentBan từ cột BAN trước (kể cả dòng tiêu đề Ban hoặc dòng chèn thêm)
          if (cols.ban !== -1) {
            const rawBan = String(row[cols.ban] || '').trim();
            if (rawBan && !rawBan.toLowerCase().includes('họ và tên') && !rawBan.toLowerCase().includes('database') && !rawBan.toLowerCase().includes('stt')) {
              currentBan = rawBan;
            }
          }

          const name = cols.name !== -1 ? String(row[cols.name] || '').trim() : '';
          if (!name || name.toLowerCase().includes('họ và tên') || name.toLowerCase().includes('database') || name.toLowerCase() === 'tên') continue;
          ...
          const memberObj = {
            ...
            rawRole,
            ban: currentBan,
            department: canonicalDept(currentBan, rawRole, role, name),
            phone,
            facebook,
            notes,
            school,
          ...
          if (!seenKeys.has(key)) {
            ...
            if (existingIdx !== -1 && (role === 'FOUNDER' || role === 'PRESIDENT' || role === 'CO_FOUNDER' || role === 'CORE' || role === 'VICE' || role === 'ADVISOR')) {
...
async function resolveFirebaseUser(req) {
  ...
  if (fb && fb.tokenFromRequest) {
    ...
    if (token) {
      try {
        ...
        console.warn('[YNDA Auth] Token verify warning:', e.message);
        decoded = null;
      }
    }
  }
  ...
  let user = null;
  try {
    user = await ensureUserSheet(decoded.uid, email, decoded.name, decoded);
  } catch (e) {
    console.warn('[YNDA Auth] ensureUserSheet warning:', e.message);
    user = null;
  }
  if (!user && email) {
    const isFounder = email === 'yniemdienanh@gmail.com';
    user = {
      USER_ID: decoded.uid || 'USR_FOUNDER',
      NAME: decoded.name || (isFounder ? 'Ban Tổ Chức · Ý Niệm Điện Ảnh' : email.split('@')[0]),
      EMAIL: email,
