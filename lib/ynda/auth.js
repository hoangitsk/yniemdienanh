...
// • Dữ liệu thành viên được tạo trong USERS khi người dùng đăng nhập Firebase
//   lần đầu. USERS là nguồn dữ liệu duy nhất cho role/ban trong YNDA.
//
...
async function ensureUserSheetInner(uid, email, name, firebaseProfile) {
  ...
  if (!user) {
    // Do not import/sync a member list from another sheet.  A Firebase login
    // creates exactly one local member record; admins can later set role/ban
    // directly in USERS without it being overwritten by an external source.
    ...
    // User đã tồn tại: chỉ liên kết Firebase UID và bổ sung thông tin hồ sơ
    // còn thiếu. Role/department luôn giữ theo USERS.
    const patch = {};
...
async function syncUsersFromRanking(membersOverride) {
  const err = new Error('Đã tắt đồng bộ từ bảng xếp hạng. USERS chỉ tạo khi thành viên đăng nhập Firebase.');
  err.code = 'USER_SYNC_DISABLED';
  throw err;
  /* c8 ignore start -- retained below temporarily as migration reference */
  const s = store.store();
