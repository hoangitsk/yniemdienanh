...
async function syncUsersFromRanking(membersOverride) {
  ...
  // Dedupe dữ liệu cũ: cùng email giữ 1 user (ưu tiên có FIREBASE_UID, rồi tới bản mới)
  const keepIds = new Set();
  const seenEmail = {};
  for (const u of existing) {
    const k = String(u.EMAIL || '').toLowerCase();
    if (!seenEmail[k]) { seenEmail[k] = u.USER_ID; keepIds.add(u.USER_ID); continue; }
    const prev = existing.find(e => e.USER_ID === seenEmail[k]);
    const prevAuto = prev && !!prev.FIREBASE_UID;
    const curAuto = !!u.FIREBASE_UID;
    if (curAuto && !prevAuto) {
      keepIds.delete(seenEmail[k]);
      seenEmail[k] = u.USER_ID;
      keepIds.add(u.USER_ID);
    }
  }
  const dedupeRemoved = existing.filter(u => !keepIds.has(u.USER_ID));
  for (const r of dedupeRemoved) await s.remove(USERS_TAB, 'USER_ID', r.USER_ID);
  if (dedupeRemoved.length) console.warn(`[YNDA] dedupe USERS: xoá ${dedupeRemoved.length} dòng trùng email`);
  const cleanExisting = existing.filter(u => keepIds.has(u.USER_ID));

  let created = 0, updated = 0;
  for (const m of members) {
    const cur = cleanExisting.find(u => String(u.EMAIL || '').toLowerCase() === m.email);
    if (!cur) {
      const userId = store.uid('USR');
