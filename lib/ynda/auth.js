...
function findRankingHeaderRow(rows) {
  ...
  for (let r = 0; r < limit; r++) {
    ...
    if (cols.name != null && cols.email != null && cols.role != null) return { row: r, cols };
  }
  return null;
}
...
async function ensureUserSheetInner(uid, email, name, firebaseProfile) {
  ...
  if (!user) {
    // Lần đầu đăng nhập bằng Firebase -> tra role/ban từ sheet ranking
    // (DATABASE CORE / DATABASE THÀNH VIÊN) theo EMAIL giống bảng xếp hạng.
    let role = 'MEMBER';
    ...
    try {
      ...
      if (match) {
        role = config.normalizeRole(match.role) || 'MEMBER';
        department = match.department || departmentFromName(match.name) || 'GLOBAL';
        rankName = match.name || '';
        rankPhone = match.phone || '';
      }
    } catch (e) {
      console.warn('[YNDA] ranking lookup failed:', e.message);
    }
    ...
    // User tạo tự động (firebase-auth / ranking-sync) được refresh role/ban
    // theo ranking; user manual giữ nguyên (không bị ghi đè).
