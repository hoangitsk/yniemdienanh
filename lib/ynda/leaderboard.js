...
async function buildLeaderboard({ seasonId, periodMonth, department, role } = {}) {
  await ensureTables();
  let users = await store().list(USERS_TAB);
  if (!users || !users.length) {
    try {
      await auth.syncUsersFromRanking();
      users = await store().list(USERS_TAB);
    } catch (e) {
      console.warn('[Leaderboard] sync ranking fallback:', e.message);
    }
  }
  ...
  let rankingPointsMap = {};
  try {
    const rankingApi = require('../../api/ranking/leaderboard');
    if (rankingApi && typeof rankingApi.readPoints === 'function') {
      const sid = process.env.SPREADSHEET_RANKING || '1SgbkoiSXP_zNYWN_AC6WKXTGQPbRAHfs2gwv_NOnsJM';
      const pts = await rankingApi.readPoints(sid);
      (pts || []).forEach(p => {
        const k = String(p.name || '').toLowerCase().trim();
        if (k) rankingPointsMap[k] = (rankingPointsMap[k] || 0) + toNumber(p.points);
      });
    }
  } catch (_) {}
  ...
  for (const u of users) {
    ...
    const userKey = String(u.NAME || '').toLowerCase().trim();
    const sheetPt = rankingPointsMap[userKey] || 0;

    const finalTaskXp = round2(b.taskXp + sheetPt);
    const finalOverall = round2(b.overall + sheetPt);
    ...
    entries.push({
      ...
      completedTasks: b.completedTasks || (sheetPt > 0 ? 1 : 0),
