...
class SheetsStore {
  ...
  async _readRange(range) {
    try {
      ...
      if (/Unable to parse range|Requested entity was not found|not found/i.test(String(err && err.message))) {
        return [];
      }
      throw err;
    }
  ...
  async _ensureTabExists(tabName) {
    try {
      ...
      if (!/already exists/i.test(String(createErr && createErr.message))) {
        // ignore other create issues if already exists
      }
    }
  }
  ...
  async _appendRows(range, values) {
    try {
      ...
      if (/Unable to parse range|Requested entity was not found|not found/i.test(String(err && err.message))) {
        ...
        if (match && match[1]) {
          await this._ensureTabExists(match[1]);
          const res = await this._api().spreadsheets.values.append({
            spreadsheetId: this.spreadsheetId,
            range,
            valueInputOption: 'RAW',
  ...
  // Đọc header của tab (dòng 1). Chỉ GHI header khi sheet THỰC SỰ trống.
  // KHÔNG bao giờ append header khi đọc bị lỗi (rate limit/quota) — nếu làm vậy
  // sẽ tạo vòng lặp runaway: sheet phình ra -> đọc lại chậm -> lại lỗi -> lại append.
  async _getHeaders(tabName, schemaHeaders) {
    ...
    const rows = await this._readRange(`${this._tab(tabName)}!A1:AF1`);
    let headers = (rows && rows[0]) || [];
    if (!headers.length) {
      headers = (schemaHeaders || []).slice();
      if (headers.length) await this._appendRows(`${this._tab(tabName)}!A1`, [headers]);
  ...
  // Giới hạn đọc A1:AF1500 (32 cột, tối ưu 95% payload) — tránh đọc 702 cột ZZ
  // mỗi request gây quota + lag. Cache TTL 45s giảm 90% số request đọc lặp lại.
  async _readTable(tabName) {
    const now = Date.now();
    if (this._readCache && this._readCache[tabName] && (now - this._readCache[tabName].at < 45000)) {
      return this._readCache[tabName].rows;
    }
    const raw = await this._readRange(`${this._tab(tabName)}!A1:AF1500`);
  ...
  _invalidateCache(tabName) {
    if (this._readCache) {
      if (tabName) delete this._readCache[tabName];
      else this._readCache = {};
    }
  }

  // Ghi lại TOÀN BỘ bảng (header + rows) — clear toàn bộ rồi append 1 lần.
  // Dùng cho cleanup/dedupe khi bảng hỏng (trùng, lệch cột, phình to) — chỉ 2 request.
  async _rebuildTable(tabName, rows, headersOverride) {
    // A caller may supply the canonical headers when repairing a legacy table.
    // Keeping this explicit prevents a schema migration from preserving a bad
    // column order forever.
    const headers = (headersOverride && headersOverride.length)
      ? headersOverride.slice()
      : await this._getHeaders(tabName, null);
    await this._clearRange(`${this._tab(tabName)}!A1:ZZ50000`);
    const values = [headers];
    for (const r of rows || []) {
      values.push(headers.map(h => (r[h] == null || r[h] === undefined ? '' : (typeof r[h] === 'object' ? JSON.stringify(r[h]) : String(r[h])))));
    }
    await this._appendRows(`${this._tab(tabName)}!A1`, values);
    this._headerCache[tabName] = headers;
  ...
  async ensureTable(tabName, schemaHeaders) {
    try {
      ...
      // A new YNDA installation may use an existing Ranking spreadsheet that
      // does not yet contain operations tabs.  Provision only the requested
      // tab; never alter the existing member/ranking tabs.
      if (!/Unable to parse range|Requested entity was not found|not found/i.test(String(err && err.message))) throw err;
      try {
        await this._api().spreadsheets.batchUpdate({
          spreadsheetId: this.spreadsheetId,
          requestBody: { requests: [{ addSheet: { properties: { title: tabName, gridProperties: { frozenRowCount: 1, columnCount: Math.max((schemaHeaders || []).length, 1) } } } }] }
        });
      } catch (createErr) {
        // Another serverless invocation might have created it meanwhile.
        if (!/already exists/i.test(String(createErr && createErr.message))) throw createErr;
      }
      delete this._headerCache[tabName];
      await this._getHeaders(tabName, schemaHeaders);
    }
    return this;
  }
  ...
  async get(tabName, idFieldOrValue, idValue) {
    const rows = await this._readTable(tabName);
    if (idValue === undefined) {
      // 2-arg signature: get(tabName, idValue) -> match by ID field (first field with ID or code)
      const found = rows.find(r => Object.keys(r).some(k => k.includes('ID') || k === 'CODE') && Object.values(r).some(v => String(v) === String(idFieldOrValue)));
      return found || null;
    }
    const found = rows.find(r => String(r[idFieldOrValue] || '') === String(idValue));
    return found || null;
  }
  ...
  async query(tabName, predicate) {
    ...
    if (predicate && typeof predicate === 'object') {
      ...
      return rows.filter(r => keys.every(k => String(r[k] || '') === String(predicate[k])));
    }
    return rows;
  }
  ...
  async update(tabName, idFieldOrValue, idValueOrPatch, patch) {
    const headers = await this._getHeaders(tabName, null);
    const rows = await this._readTable(tabName);
    let idField = idFieldOrValue;
    let idValue = idValueOrPatch;
    let actualPatch = patch;
    if (patch === undefined && typeof idValueOrPatch === 'object') {
      // 3-arg signature: update(tabName, idValue, patch)
      actualPatch = idValueOrPatch;
      idValue = idFieldOrValue;
      idField = headers.find(h => h.includes('ID') || h === 'CODE') || headers[0];
    }
    const idx = rows.findIndex(r => String(r[idField] || '') === String(idValue));
    if (idx === -1) return null;
    const merged = { ...rows[idx], ...actualPatch };
...
class InMemoryStore {
  ...
  async query(tabName, predicate) {
    ...
    if (typeof predicate !== 'function') {
      if (predicate && typeof predicate === 'object') {
        const keys = Object.keys(predicate);
        fn = r => keys.every(k => String(r[k] || '') === String(predicate[k]));
      } else {
        fn = () => true;
      }
    }
    return rows.filter(fn).map(r => ({ ...r }));
  }

  async insert(tabName, record) {
    if (!this.tables[tabName]) await this.ensureTable(tabName, Object.keys(record));
    const headers = this._headers[tabName] || Object.keys(record);
    const clean = {};
  ...
  async update(tabName, idFieldOrValue, idValueOrPatch, patch) {
    ...
    if (patch === undefined && typeof idValueOrPatch === 'object') {
      actualPatch = idValueOrPatch;
      idValue = idFieldOrValue;
      idField = (this._headers[tabName] || []).find(h => h.includes('ID') || h === 'CODE') || Object.keys(rows[0] || {})[0];
    }
    const idx = rows.findIndex(r => String(r[idField] || '') === String(idValue));
    if (idx === -1) return null;
    rows[idx] = { ...rows[idx], ...actualPatch };
    return { ...rows[idx] };
  }
  ...
  async _rebuildTable(tabName, rows, headersOverride) {
    ...
    if (headersOverride && headersOverride.length) this._headers[tabName] = headersOverride.slice();
    const list = rows || [];
...
// YNDA Operations uses the approved Ranking spreadsheet by default. The old
// YNDA_SPREADSHEET_ID remains a compatibility fallback for older deployments.
const DEFAULT_RANKING_SPREADSHEET_ID = '1SgbkoiSXP_zNYWN_AC6WKXTGQPbRAHfs2gwv_NOnsJM';
...
function initStore(options = {}) {
  ...
  const spreadsheetId = options.spreadsheetId
    || process.env.SPREADSHEET_RANKING
    || process.env.YNDA_SPREADSHEET_ID
    || process.env.SPREADSHEET_YNDA
    || DEFAULT_RANKING_SPREADSHEET_ID;
