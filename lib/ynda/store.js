...
class SheetsStore {
  ...
  async _getHeaders(tabName, schemaHeaders) {
    ...
    const rows = await this._readRange(`${this._tab(tabName)}!A1:ZZ1`);
  ...
  // Giới hạn đọc 3000 dòng (realistic cho mọi bảng YNDA) — tránh đọc 50k dòng
  // mỗi request gây quota + runaway. Cache TTL 10s giảm số request đọc.
  async _readTable(tabName) {
    const now = Date.now();
    if (this._readCache && this._readCache[tabName] && now - this._readCache[tabName].at < 10000) {
      return this._readCache[tabName].rows;
    }
    const raw = await this._readRange(`${this._tab(tabName)}!A1:ZZ3000`);
