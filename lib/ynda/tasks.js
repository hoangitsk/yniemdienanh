...
async function getTask(ref) {
  await ensureTables();
  const s = store();
  const byId = await s.get(TAB, ref);
  if (byId) return byId;
  const list = await s.query(TAB, { CODE: String(ref).toUpperCase() });
  return list[0] || null;
}
...
async function updateTask(taskId, patch) {
  ...
  return s.update(TAB, taskId, clean);
}
...
async function listTasks(filter = {}) {
  ...
  if (filter.createdBy) q.CREATED_BY = filter.createdBy;
  if (filter.templateId) q.TEMPLATE_ID = filter.templateId;
  const list = await s.query(TAB, q);
  return list;
}
