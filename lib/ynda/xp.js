...
// Danh sách transaction được áp dụng (Applied) của user (theo mùa và tháng nếu có)
async function listAppliedTxns(userId, seasonId, periodMonth) {
  ...
  return rows.filter(r => {
    ...
    if (periodMonth) {
      ...
      if (!timeStr.startsWith(periodMonth)) return false;
    }
    return true;
  });
}
...
async function listUserTxns(userId, opts = {}) {
  ...
  if (opts.periodMonth) {
    out = out.filter(r => String(r.TIME || '').startsWith(opts.periodMonth));
  }
  if (opts.status) {
...
async function computeBreakdown(userId, opts = {}) {
  const txns = await listAppliedTxns(userId, opts.seasonId, opts.periodMonth);
  ...
  for (const t of txns) {
    ...
    if (type === config.XP_TYPES.TASK || type === config.XP_TYPES.INTERACTION) {
      taskXp += applied;
      if (applied > 0 && t.SOURCE_ID) {
        completedTaskIds.add(String(t.SOURCE_ID));
      }
  ...
  if (completedCount === 0 && taskXp > 0) {
    completedCount = Math.max(1, Math.round(taskXp / 1.5));
  }

  // Quality score: tính theo tỷ lệ hoàn thành tốt và bonus
  let qualityScore = 95;
  if (penaltyXp < 0) {
    const penaltyRatio = Math.min(0.5, Math.abs(penaltyXp) / Math.max(1, taskXp + bonusXp));
    qualityScore = Math.max(60, Math.round(95 - penaltyRatio * 60));
  } else if (bonusXp > 0) {
    qualityScore = Math.min(100, Math.round(95 + Math.min(5, bonusXp)));
  }

  const base = taskXp + bonusXp + penaltyXp;
  let overall = base;
  const isRoleHolder = [
    config.ROLES.CORE, config.ROLES.VICE, config.ROLES.FOUNDER,
    config.ROLES.PRESIDENT, config.ROLES.CO_FOUNDER, config.ROLES.CORE_FOUNDER
  ].includes(role);
...
async function computeAllBreakdowns(seasonId, periodMonth) {
  const users = await store().list(USERS_TAB);
  const output = [];
  for (const u of users) {
    if (String(u.STATUS || '') === 'ARCHIVED') continue;
    const b = await computeBreakdown(u.USER_ID, { seasonId, periodMonth });
    output.push({ ...u, ...b });
  }
  return output;
}
